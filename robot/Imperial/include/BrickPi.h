...
#define US_I2C_SPEED 20 //#tweak this value
#define US_I2C_IDX 0
...
int BrickPiUpdateValues(bool getUltrasonic = true){
  unsigned char i = 0;
  unsigned int ii = 0;
  while(i < 2){
    Retried = 0;
...
//    AddBits(1, 0, 2, 0);     use this to disable encoder offset
	...
	if(BrickPi.SensorType[port] == TYPE_SENSOR_ULTRASONIC_CONT){
		BrickPi.SensorI2CWrite[port][US_I2C_IDX] = getUltrasonic ? 1 : 0;
		BrickPi.SensorI2CRead [port][US_I2C_IDX] = getUltrasonic ? 1 : 0;
	}
        unsigned char device = 0;
        while(device < BrickPi.SensorI2CDevices[port]){
          if(!(BrickPi.SensorSettings[port][device] & BIT_I2C_SAME)){
	...
	case TYPE_SENSOR_ULTRASONIC_CONT:{
	  ...
	  if (BrickPi.SensorType[port] == TYPE_SENSOR_ULTRASONIC_CONT ){
		if(getUltrasonic && BrickPi.Sensor[port] & ( 0x01 << US_I2C_IDX))
...
int BrickPiRx(unsigned char *InBytes, unsigned char *InArray, long timeout){  // timeout in uS, not mS
  ...
  while(serialDataAvail(UART_file_descriptor) <= 0){
    if(timeout && ((CurrentTickUs() - OrigionalTick) >= timeout))
      return -2;
    usleep(1000); // sleutene: give other threads the possibility to do stuff while waiting!!
  }
